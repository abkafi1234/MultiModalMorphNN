"""C20 — Fine-tuned EfficientNet-B3, 15 epochs + PCA-128 + RF.
EfficientNet-B3 is larger than MobileNetV3 (1536-dim features vs 960-dim).
Hypothesis: richer feature space → better class separation.
AdamW(lr=1e-4), CosineAnnealingWarmRestarts(T_0=5), label_smoothing=0.05.
"""
...
def finetune_b3(imgs_tr, y_in, epochs=EPOCHS):
...
for rep in range(CV_REP_CNN):
    ...
    for fold, (tr_i, vl_i) in enumerate(skf.split(X_morph_tr, y_tr)):
        ...
        feat_ext, pool = finetune_b3(list(flat_tr_imgs[tr_i]), y_tr[tr_i])
...
feat_ext_h, pool_h = finetune_b3(list(flat_tr_imgs), y_tr)
