"""C13 — Fine-tuned MobileNetV3, NO PCA compression + RF.
MobileNetV3 produces 960-dim embeddings. We feed ALL dims directly to RF.
Hypothesis: PCA-64/128 still discards discriminative variance; removing
compression entirely lets RF see everything the fine-tuned backbone learned.
16 morph + 960 CNN = 976-dim hybrid vector.
"""
import sys; sys.path.insert(0, str(__import__('pathlib').Path(__file__).resolve().parents[1]))
from shared_utils import *
from sklearn.ensemble import RandomForestClassifier

CFG      = "C13_ft_v3_nopca_rf"
...
print(f"\n{'='*55}\n{CFG}  |  16 morph + 960 FT-CNN (no PCA, {BACKBONE}) + RF\n{'='*55}")
...
for rep in range(CV_REP_CNN):
    ...
    for fold, (tr_i, vl_i) in enumerate(skf.split(X_morph_tr, y_tr)):
        ...
        # No PCA — use raw embeddings with just StandardScaler
        sc  = StandardScaler()
        Xf_tr = sc.fit_transform(np.hstack([X_morph_tr[tr_i], Xc_tr_ft]))
        Xf_vl = sc.transform(       np.hstack([X_morph_tr[vl_i], Xc_vl_ft]))
...
sc_h = StandardScaler()
Xf_tr_h = sc_h.fit_transform(np.hstack([X_morph_tr, Xc_tr_h]))
Xf_te_h = sc_h.transform(       np.hstack([X_morph_te, Xc_te_h]))
