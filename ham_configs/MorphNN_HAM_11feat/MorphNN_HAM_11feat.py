"""MorphNN-HAM-11feat — Same as MorphNN-HAM but using only the 11 morphological
features that are domain-aligned with single-lesion dermoscopy.

Dropped (5 multi-lesion features, indices 0-based):
  idx 0  f1  lesion_count          — always ~200+ due to oversegmentation
  idx 4  f5  sparsity_score        — single-lesion has no spatial spread
  idx 5  f6  confluence_density    — undefined for 1 connected region
  idx 13 f14 spatial_entropy       — trivial for 1-lesion centroid
  idx 14 f15 max_lesion_area_ratio — always ~0 due to oversegmentation

Kept (11 shape/colour/size features):
  idx 1  f2  avg_lesion_area
  idx 2  f3  area_heterogeneity
  idx 3  f4  avg_circularity
  idx 6  f7  localized_hue
  idx 7  f8  localized_saturation
  idx 8  f9  avg_aspect_ratio
  idx 9  f10 avg_solidity
  idx 10 f11 localized_value
  idx 11 f12 hue_std
  idx 12 f13 saturation_std
  idx 15 f16 background_saturation
"""
import sys, os
...
CNN_BATCH = 32
AUG_TARGET = 2000
CFG        = "MorphNN_HAM_11feat"

# The 11 aligned feature indices (0-based, out of 16 total)
ALIGNED_IDX = [1, 2, 3, 6, 7, 8, 9, 10, 11, 12, 15]
N_MORPH     = len(ALIGNED_IDX)  # 11

ALIGNED_NAMES = [
    "avg_lesion_area", "area_heterogeneity", "avg_circularity",
    "localized_hue", "localized_saturation", "avg_aspect_ratio",
    "avg_solidity", "localized_value", "hue_std",
    "saturation_std", "background_saturation",
]

print(f"\n{'='*60}")
print(f"{CFG}  |  HAM10000 — 11 aligned morph features")
print(f"{'='*60}")
print(f"Using {N_MORPH} aligned features: {ALIGNED_NAMES}")
...
# ── Joint model with 11 morph inputs ─────────────────────────────────────────
class JointMorphCNN11(nn.Module):
    def __init__(self):
        super().__init__()
        base = models.mobilenet_v3_large(weights=models.MobileNet_V3_Large_Weights.DEFAULT)
        self.features = base.features
        self.avgpool  = base.avgpool
        self.morph_fc = nn.Sequential(nn.Linear(N_MORPH, 64), nn.Hardswish(), nn.Dropout(0.3))
...
def finetune_joint(imgs_tr, morphs_tr, y_in, epochs=EPOCHS):
    ...
    model = JointMorphCNN11().to(device)
...
X_morph_tr, y_tr = load_morph_aligned("train")
X_morph_te, y_te = load_morph_aligned("test")
print(f"Aligned morph train: {X_morph_tr.shape}, test: {X_morph_te.shape}")
...
print("\nFull train for holdout evaluation ...")
...
ho_macro  = f1_score(y_te, y_pred, average="macro")
ho_weighted = f1_score(y_te, y_pred, average="weighted")
ho_acc    = accuracy_score(y_te, y_pred)
errors    = int((y_pred != y_te).sum())
rep_text  = classification_report(y_te, y_pred, target_names=CLASSES, digits=4)

row = {
    "Config":          CFG,
    "N_morph_features": N_MORPH,
    "CV_MacroF1":      round(m, 4),
    "CV_Std":          round(float(np.array(cv_scores).std(ddof=1)), 4),
    "CI95":            f"[{lo:.4f},{hi:.4f}]",
    "Holdout_MacroF1": round(ho_macro, 4),
    "Holdout_WeightedF1": round(ho_weighted, 4),
    "Holdout_Accuracy": round(ho_acc, 4),
    "Errors":          errors,
    "N_test":          len(y_te),
    "ErrorRate_pct":   round(100*errors/len(y_te), 2),
}
pd.DataFrame([row]).to_csv(RES/"summary.csv", index=False)
np.save(RES/"y_pred.npy", y_pred)
np.save(RES/"y_test.npy", y_te)
np.save(RES/"cv_scores.npy", np.array(cv_scores))
with open(RES/"classification_report.txt","w") as f:
...
# Load original 16-feat results for comparison
orig_res = Path(__file__).resolve().parents[2] / "ham_configs" / "C21_joint_morphcnn" / "results"
y_pred_orig = np.load(orig_res / "y_pred.npy")
y_test_orig = np.load(orig_res / "y_test.npy")
orig_macro = f1_score(y_test_orig, y_pred_orig, average="macro")
orig_acc   = accuracy_score(y_test_orig, y_pred_orig)
orig_errors = int((y_pred_orig != y_test_orig).sum())
print(f"\n{'='*60}")
print(f"COMPARISON: 16-feat vs 11-feat MorphNN on HAM10000 holdout")
print(f"{'='*60}")
print(f"  MorphNN-16feat:  Macro F1={orig_macro:.4f}  Acc={orig_acc:.4f}  Errors={orig_errors}")
print(f"  MorphNN-11feat:  Macro F1={ho_macro:.4f}  Acc={ho_acc:.4f}  Errors={errors}")
delta_f1  = ho_macro - orig_macro
delta_err = errors - orig_errors
print(f"  ΔMacro F1 = {delta_f1:+.4f}")
print(f"  ΔErrors   = {delta_err:+d}")
